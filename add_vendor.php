...
$get_vendor=$obj->getvendor_details();
...
$data=$obj->create_vendor($companyname,$name,$lname,$mbno,$code,$shift,$address,$newFilePath,$password,$email,$newFilePath2,$dno);

if($data==1)
{
//echo "<script> alert('Security Created Successfully');</script>";
header("location:add_vendor.php?sadd=success");
}
...
<div id="main" class="container-fluid">
	<div class="row">
		...
		<a href="#" class="add_page_button">Add Vendor <span class="fa fa-plus"></span> </a>
		...
		{
		echo "Vendor Staff Added Successfully";
		}
			if($_REQUEST['cadd']=="success")
		{
		echo "Vendor Staff Details Updated Successfully";
		}
		?>
		</span>
 ...
 <div id="locationField">
  ...
  background-color: #fff;  background-position: right center;  background-size: 20px;"/>
			 ...
			 <br>
				...
				if($get_vendor>0)
				{
				while($get_rec=mysql_fetch_array($get_vendor))
				{
							
				?>
	<tr>
          <td><?php echo $get_rec['vendor_name']; ?></td>
		  <td><?php echo $get_rec['vendor_lname']; ?></td>
		  <td><?php echo $get_rec['vendor_cat']; ?></td>
		  <td><?php echo $get_rec['vendor_mobile']; ?></td>
		  
		<td>
		<a href="javascript:" class="owner" onClick="editButton('<?php echo $get_rec['vendor_id']; ?>')">
<!--<i class="fa fa-edit"></i> -->Edit</a>
						<td><i class="fa fa-times"  onclick="fundel('<?php echo $get_rec['vendor_id'];?>'); "></i></td></tr>
						<?php }}?>
						</table>
...
function editButton(id) {
	//alert(id);
$('#sessiondata23').load('vendor_id.php?vendorid='+id);
window.location.replace('edit_vendor.php');
...
{
//alert(delid);
$.post("delete_vendor.php",{sec_id:delid},function(data) { location.reload();
//$(".delresult").html(data);
//alert(data);
});
